...
print("KINODYNAMICS MODEL:\n\
      x = [q("+str(nq)+"), q_dot("+str(nv)+"]\n\
      u = [ddq_act("+str(nv-6)+"), wrench_left(6), wrench_right(6)]")
...
foottraj.updateForward(0, x_forward, y_gap, y_forward, -0.01, z_height, swing_apex)
lowlevel_time = 0
...
for t in range(Tmpc):
    ...
    problem.removeTerminalConstraints()
    problem.addTerminalConstraint(term_constraint_com)
